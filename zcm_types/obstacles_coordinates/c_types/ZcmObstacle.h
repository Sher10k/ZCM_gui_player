// THIS IS AN AUTOMATICALLY GENERATED FILE.  DO NOT MODIFY
// BY HAND!!
//
// Generated by zcm-gen

#include <stdint.h>
#include <stdlib.h>
#include <zcm/zcm_coretypes.h>
#include <zcm/zcm.h>

#ifndef _ZcmObstacle_h
#define _ZcmObstacle_h

#ifdef __cplusplus
extern "C" {
#endif

#include "ZcmObstacleProperties.h"
#include "ZcmObstacleLocalCoordinates.h"
#include "ZcmObstacleUtmCoordinates.h"
#include "ZcmObstacleGeoCoordinates.h"
#define ZCMOBSTACLE_STATIC_OBJECT 0
#define ZCMOBSTACLE_MOVING_OBJECT 1

/**
 * @struct ZcmObstacle
 * @brief  Данные об объекте
 * @var ZcmObstacle::cluster_id 
 * @brief - номер объекта
 * @var ZcmObstacle::object_type
 * @brief - тип объекта
 * @var ZcmObstacle::object_properies
 * @brief - свойства объекта
 * @var ZcmObstacle::local_coordinates
 * @brief - координаты в локальной системе
 * @var ZcmObstacle::utm_coordinates
 * @brief - координаты в UTM
 * @var ZcmObstacle::geo_coordinates
 * @brief - географические координаты
 *
 */
typedef struct _ZcmObstacle ZcmObstacle;
struct _ZcmObstacle
{
    int32_t    cluster_id;
    int32_t    object_type;
    ZcmObstacleProperties object_properies;
    ZcmObstacleLocalCoordinates local_coordinates;
    ZcmObstacleUtmCoordinates utm_coordinates;
    ZcmObstacleGeoCoordinates geo_coordinates;
};

/**
 * Create a deep copy of a ZcmObstacle.
 * When no longer needed, destroy it with ZcmObstacle_destroy()
 */
ZcmObstacle* ZcmObstacle_copy(const ZcmObstacle* to_copy);

/**
 * Destroy an instance of ZcmObstacle created by ZcmObstacle_copy()
 */
void ZcmObstacle_destroy(ZcmObstacle* to_destroy);

/**
 * Identifies a single subscription.  This is an opaque data type.
 */
typedef struct _ZcmObstacle_subscription_t ZcmObstacle_subscription_t;

/**
 * Prototype for a callback function invoked when a message of type
 * ZcmObstacle is received.
 */
typedef void(*ZcmObstacle_handler_t)(const zcm_recv_buf_t* rbuf,
             const char* channel, const ZcmObstacle* msg, void* userdata);

/**
 * Publish a message of type ZcmObstacle using ZCM.
 *
 * @param zcm The ZCM instance to publish with.
 * @param channel The channel to publish on.
 * @param msg The message to publish.
 * @return 0 on success, <0 on error.  Success means ZCM has transferred
 * responsibility of the message data to the OS.
 */
int ZcmObstacle_publish(zcm_t* zcm, const char* channel, const ZcmObstacle* msg);

/**
 * Subscribe to messages of type ZcmObstacle using ZCM.
 *
 * @param zcm The ZCM instance to subscribe with.
 * @param channel The channel to subscribe to.
 * @param handler The callback function invoked by ZCM when a message is received.
 *                This function is invoked by ZCM during calls to zcm_handle() and
 *                zcm_handle_timeout().
 * @param userdata An opaque pointer passed to @p handler when it is invoked.
 * @return pointer to subscription type, NULL if failure. Must clean up
 *         dynamic memory by passing the pointer to ZcmObstacle_unsubscribe.
 */
ZcmObstacle_subscription_t* ZcmObstacle_subscribe(zcm_t* zcm, const char* channel, ZcmObstacle_handler_t handler, void* userdata);

/**
 * Removes and destroys a subscription created by ZcmObstacle_subscribe()
 */
int ZcmObstacle_unsubscribe(zcm_t* zcm, ZcmObstacle_subscription_t* hid);
/**
 * Encode a message of type ZcmObstacle into binary form.
 *
 * @param buf The output buffer.
 * @param offset Encoding starts at this byte offset into @p buf.
 * @param maxlen Maximum number of bytes to write.  This should generally
 *               be equal to ZcmObstacle_encoded_size().
 * @param msg The message to encode.
 * @return The number of bytes encoded, or <0 if an error occured.
 */
int ZcmObstacle_encode(void* buf, uint32_t offset, uint32_t maxlen, const ZcmObstacle* p);

/**
 * Decode a message of type ZcmObstacle from binary form.
 * When decoding messages containing strings or variable-length arrays, this
 * function may allocate memory.  When finished with the decoded message,
 * release allocated resources with ZcmObstacle_decode_cleanup().
 *
 * @param buf The buffer containing the encoded message
 * @param offset The byte offset into @p buf where the encoded message starts.
 * @param maxlen The maximum number of bytes to read while decoding.
 * @param msg Output parameter where the decoded message is stored
 * @return The number of bytes decoded, or <0 if an error occured.
 */
int ZcmObstacle_decode(const void* buf, uint32_t offset, uint32_t maxlen, ZcmObstacle* msg);

/**
 * Release resources allocated by ZcmObstacle_decode()
 * @return 0
 */
int ZcmObstacle_decode_cleanup(ZcmObstacle* p);

/**
 * Check how many bytes are required to encode a message of type ZcmObstacle
 */
uint32_t ZcmObstacle_encoded_size(const ZcmObstacle* p);
uint32_t ZcmObstacle_struct_size(void);
uint32_t ZcmObstacle_num_fields(void);
int      ZcmObstacle_get_field(const ZcmObstacle* p, uint32_t i, zcm_field_t* f);
const zcm_type_info_t* ZcmObstacle_get_type_info(void);

// ZCM support functions. Users should not call these
int64_t  __ZcmObstacle_get_hash(void);
uint64_t __ZcmObstacle_hash_recursive(const __zcm_hash_ptr* p);
int      __ZcmObstacle_encode_array(void* buf, uint32_t offset, uint32_t maxlen, const ZcmObstacle* p, uint32_t elements);
int      __ZcmObstacle_decode_array(const void* buf, uint32_t offset, uint32_t maxlen, ZcmObstacle* p, uint32_t elements);
int      __ZcmObstacle_decode_array_cleanup(ZcmObstacle* p, uint32_t elements);
uint32_t __ZcmObstacle_encoded_array_size(const ZcmObstacle* p, uint32_t elements);
uint32_t __ZcmObstacle_clone_array(const ZcmObstacle* p, ZcmObstacle* q, uint32_t elements);

#ifdef __cplusplus
}
#endif

#endif
